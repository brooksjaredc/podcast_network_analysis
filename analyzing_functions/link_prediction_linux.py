...
df1 = pd.read_csv('guest_host_cleaned_podcasts.csv', sep='\t', index_col=0)
...
for index, row in p_info.iterrows():
	...
	for person in people:
		...
		# else:
		# 	print(person)
	filename = 'link_pred_data_' + str(index) + '.csv'
	print(filename)

	link_pred_data.to_csv(filename, sep='\t')




# link_pred_data.to_csv('link_pred_data_1of10.csv', sep='\t')
